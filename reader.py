    ...
    while (ch != "q"):
        ...
        for line in range(0, feedListDims[2] if len(feeds) > feedListDims[2] else len(feeds)):
            ...
            if line == selectedFeed:          
                feedListWin.addnstr(feeds[0 if selectedFeed>=feedListDims[2] else selectedFeed - feedListDims[2]:][line], feedListDims[3], curses.A_STANDOUT)
            else:
                feedListWin.addnstr(feeds[line], feedListDims[3])

        feedContentWin.move(0, 0)
        feedContentWin.addnstr(lorem[textLine*feedContentDims[3]:], feedContentDims[2]*feedContentDims[3]-1) #TODO: Ist das so richtig?
        ...
        if ch == "KEY_LEFT" and activeWin == feedContentWin:
            ...
            textLine -= 1
        elif ch == "KEY_DOWN" and activeWin == feedContentWin and textLine < len(lorem)/feedContentDims[3]-feedContentDims[2]: #TODO: Anzahl Zeilen zählen
            textLine += 1
